...
let theUrl = "http://api.currencylayer.com/live?access_key=2243dba5ea20e9d54f41fc7da3486f04&currencies=USD,INR,JPY";
xmlHttp.open("GET", theUrl, false);
xmlHttp.send(null);
let response = JSON.parse(xmlHttp.responseText);
let currency = response.quotes;
let currentCurrency = "INR";
let selectedOption,vehicleType,inputField;
...
//Function to display initial message and input name field.
...
function employeeRegForm(){
    ...
    switch(inputField){
        ...
        case 'confirmPassword' :
            if (confirmPassword()){
                messageField.innerHTML = `Hi ${employeeName}! May I know your contact number?`;
        ...
        case 'contact' :
            if (validateContact()){
                messageField.innerHTML = `On submitting '${employeeName}' will be added as employee!`;
        ...
        case 'vehicleNumber':
            ...
            break;
        case 'description':
            messageField.innerHTML = `On submitting vehicle '${vehicleName}' will be added!`;
...
function validateEmployeeGender(){
    if (((document.getElementById('maleRadio').checked) == false) && ((document.getElementById('femaleRadio').checked) == false) && ((document.getElementById('otherRadio').checked) == false)) {
...
function validateVehicleType(){
    ...
    if ((selectedOption != 'cycle') && (selectedOption != 'motorcycle') && (selectedOption != 'fourwheeler')) {
        ...
        document.getElementById('dailyPass').value = dailyValue;
        document.getElementById('monthlyPass').value = monthlyValue;
        document.getElementById('yearlyPass').value = yearlyValue;
        return true;
    }
}
