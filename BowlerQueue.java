...
public class BowlerQueue implements BowlerQueueInterface{
	...
	//constructor
	public BowlerQueue(int sizeOfQueue) throws Exception{
		if (sizeOfQueue<=0){
	...
	public void enQueue(Bowler bowler) throws Exception {
		...
		if ((front==-1) && (rear==-1)){
			...
			System.out.println(bowler.getNumberOfDeliveries()+" deliveries added for bowler "+bowler.getNameOfBowler());
		}
		else {
			rear = rear+1;
			priorityQueue[rear] = bowler;
	 ...
	 * Creating the max heap for number of balls per bowler
	 */
	...
	public void show (){
		for (int i=0;i<rear;i++){
			System.out.println(i+". "+priorityQueue[i].getNameOfBowler() + " "+priorityQueue[i].getNumberOfDeliveries());
		}
	}
}